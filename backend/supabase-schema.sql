...
-- Warning: this drops the app tables listed below before recreating them.
...
drop table if exists guest_report_usages cascade;
...
drop function if exists set_incident_reference_code() cascade;
...
create table users (
    ...
    volunteer_skills jsonb null,
    volunteer_availability boolean not null default false,
...
create index users_is_volunteer_index on users(is_volunteer);
create index users_volunteer_availability_index on users(volunteer_availability);
...
create index emergency_profiles_is_public_updated_at_index
    on emergency_profiles(is_public, updated_at);
...
create table incidents (
    ...
    ai_risk_score smallint not null default 0 check (ai_risk_score between 0 and 100),
    created_at timestamp(0) without time zone null default current_timestamp,
    updated_at timestamp(0) without time zone null default current_timestamp,
...
create index incidents_ai_risk_created_idx on incidents(ai_risk_score, created_at);
...
create index incident_assignments_is_volunteer_index
    on incident_assignments(is_volunteer);
...
create index hazard_zones_type_is_active_index
    on hazard_zones(type, is_active);
create index hazard_zones_is_active_created_at_index
    on hazard_zones(is_active, created_at);
...
create index broadcasts_target_type_created_at_index
    on broadcasts(target_type, created_at);
create index broadcasts_target_barangay_created_at_index
    on broadcasts(target_barangay, created_at);
create index broadcasts_sent_by_created_at_index
    on broadcasts(sent_by, created_at);
...
create index broadcast_recipients_user_id_is_read_created_at_index
    on broadcast_recipients(user_id, is_read, created_at);

create table feedback_ratings (
    id uuid primary key default gen_random_uuid(),
    incident_id uuid not null references incidents(id) on delete cascade,
    user_id uuid not null references users(id) on delete cascade,
    responder_id uuid null references users(id) on delete set null,
...
create index feedback_ratings_responder_id_created_at_index
    on feedback_ratings(responder_id, created_at);
create index feedback_ratings_rating_created_at_index
    on feedback_ratings(rating, created_at);
create index feedback_ratings_user_id_created_at_index
    on feedback_ratings(user_id, created_at);
...
create index sos_alerts_status_created_at_index
    on sos_alerts(status, created_at);
create index sos_alerts_user_id_created_at_index
    on sos_alerts(user_id, created_at);
...
('2026_04_23_010000_add_supabase_performance_indexes', 1),
('2026_04_23_020000_add_guest_reporting_support', 1),
('2026_04_25_000000_add_client_uuid_to_incidents_table', 1),
('2026_05_10_000000_create_sos_alerts_table', 1),
('2026_05_10_010000_create_hazard_zones_table', 1),
('2026_05_10_020000_add_ai_risk_score_to_incidents_table', 1),
('2026_05_10_030000_add_volunteer_fields_to_users_and_assignments_table', 1),
('2026_05_10_040000_create_broadcasts_tables', 1);
